      ...
      {ads.length === 0 ? (<p className="text-gray-500">Belum ada slot iklan.</p>) : (
        <>
          ...
          <div className="md:hidden space-y-3">
            {ads.map((ad) => (
              <div key={ad.id} className="bg-white border border-gray-200 rounded-xl p-4 flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-gray-900">{ad.name}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{ad.position} · {ad.slotId || 'No slot ID'}</p>
                </div>
                <button onClick={() => toggleActive(ad.id, ad.isActive)} className={`text-xs px-3 py-1.5 rounded-full whitespace-nowrap ${ad.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                  {ad.isActive ? 'Aktif' : 'Nonaktif'}
                </button>
              </div>
            ))}
